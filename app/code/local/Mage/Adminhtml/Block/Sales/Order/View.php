...
class Mage_Adminhtml_Block_Sales_Order_View extends Mage_Adminhtml_Block_Widget_Form_Container
{
    ...
    public function __construct()
    {
        ...
        if ($this->_isAllowedAction('ship') && $order->canShip()
            && !$order->getForcedDoShipmentWithInvoice() && $order->getStatus() == 'order_confirm') {
        ...
        //Alex
        $confirmmsg = '您确认要审核确认该订单吗？';
                  $onclickJs = 'deleteConfirm(\''
                . $confirmmsg
                . '\', \'' . $this->getConfirmUrl() . '\');';
        if($order->getStatus() == 'wait_seller_send_goods'){
            $this->_addButton('service_confirm', array(
                'label'     => Mage::helper('sales')->__('审核确认'),
                'onclick'   => $onclickJs,
                'class'     => 'go'
            ));
        }
    ...
    {
        return $this->getUrl('*/sales_order/confirm');
    }
