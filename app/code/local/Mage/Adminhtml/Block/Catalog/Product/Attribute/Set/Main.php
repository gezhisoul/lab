...
class Mage_Adminhtml_Block_Catalog_Product_Attribute_Set_Main extends Mage_Adminhtml_Block_Template
{
    ...
    public function getGroupTreeJson()
    {
        ...
        foreach ($groups as $node) {
            if($node->getAttributeGroupName() == 'Recurring Profile'){
                continue;
            }
            $item = array();
            //Alex
            ...
            if ($nodeChildren->getSize() > 0) {
                ...
                foreach ($nodeChildren->getItems() as $child) {
                    if($child->getFrontendLabel() == 'In feed' || $child->getFrontendLabel() == 'Tax Class' || $child->getFrontendLabel() == 'Is product available for purchase with Google Checkout' || $child->getFrontendLabel() == 'Allow Gift Message' || $child->getFrontendLabel() == 'Short Description'){
                             continue;
                     }
