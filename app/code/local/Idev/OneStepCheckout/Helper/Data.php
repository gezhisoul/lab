...
class Idev_OneStepCheckout_Helper_Data extends Mage_Core_Helper_Abstract
{
    public function setCustomerComment($observer)
    {
        ...
        $district = Mage::getSingleton('checkout/type_onepage')->getQuote()->getShippingAddress()->getDistrict();
        $observer->getEvent()->getOrder()->getShippingAddress()->setFax($district);
       // Mage::getSingleton('checkout/type_onepage')->getQuote()->getShippingAddress()->setFax($district)->save();
       // Mage::getSingleton('checkout/type_onepage')->getQuote()->getBillingAddress()->setFax($district)->save();
